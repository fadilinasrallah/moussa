...
\small Plan LSB bleu
\end{minipage}
\caption{Décomposition réelle de l'image utilisée pour la démonstration : canaux RGB puis plans LSB. Les plans LSB ressemblent à du bruit, mais c'est précisément la couche manipulée par l'insertion.}
\end{figure}
...
\end{minipage}
\caption{Comparaison de l'image propre, de l'image contenant le texte caché et de la différence amplifiée. Les deux premières images sont presque identiques à l'oeil, tandis que l'amplification révèle les valeurs RGB modifiées.}
\end{figure}
...
\caption{Exemple visuel généré pendant les tests : différence amplifiée après insertion LSB. Cette image aide à expliquer que la modification est spatialement localisée dans les pixels, même si elle reste très faible dans l'image normale.}
\end{figure}
